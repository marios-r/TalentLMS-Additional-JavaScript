$(function(){
	if($('#tl-list-user-groups_wrapper').length==1){
		...
		$(document).on('click', '.massaction',function(){
			...
			var url="user/listgroup/id:"+userid;
			$.ajax({
				dataType: "json",
				url: url,
				success: function (data) {
					...
					$.each( data.data, function( index ){
						var operation=$(this[2]).attr('title');
						if(operation==action){
							var dataurl=$(this[2]).data().url;
							$.ajax({
								dataType: "json",
								url:dataurl
								})
